...
export default function App() {
  ...
  const handleCopy = async () => {
    ...
    setIsCopying(true);
    try {
      const finalHtml = await makeWeChatCompatible(renderedHtml, activeTheme);
  ...
  const handleExportHtml = () => {
    ...
    a.href = url;
    a.download = `Garry_Article_${new Date().getTime()}.html`;
    a.click();
  ...
  const handleExportPdf = () => {
    ...
    const opt = {
      margin: 10,
      filename: `Garry_Article_${new Date().getTime()}.pdf`,
